...
export default function UserCard({user,error,loading}){
    if (loading) {
        return (
            <article className="w-full h-auto flex justify-center p-8 sm:p-12 dark:bg-Neutral-800 bg-Neutral-0 rounded-2xl drop-shadow-xl opacity-50">
                <img
                    className="w-18 h-18 sm:w-29 sm:h-29 rounded-full animate-pulse"
                    src={user.avatar}
                    alt="User profile"
                />
            </article>
    ...
    if (error ) {
        return(
            <div className="w-full max-w-182.5 h-auto py-8 px-6 dark:bg-Neutral-800 bg-Neutral-0 rounded-2xl text-center space-y-4 drop-shadow-xl ">
                <h2 className="dark:text-Neutral-0 text-Neutral-700 font-bold text-2xl">No results found!</h2>
                <p className="dark:text-Neutral-200 text-Neutral-300 text-[15px]">We couldn’t find any GitHub users matching your search. Please double-check the username and try again.</p>
            </div>
    
        )
    }
    return(
        <article className="w-full max-w-182.5 h-auto py-8 px-6 sm:p-12 dark:bg-Neutral-800 bg-Neutral-0 rounded-2xl space-y-8 drop-shadow-xl ">
            <div className="grid grid-cols-1 sm:grid-cols-[auto_1fr] gap-6 sm:gap-10  relative ">
                <img
                    className="w-18 h-18 sm:w-29 sm:h-29 rounded-full"
                    src={user.avatar}
                    alt="User profile"
                />
                <div>
                    <header className="flex top-0 right-0  gap-6 sm:gap-10 absolute sm:relative  ">
                        <div className="flex-1 sm:flex sm:flex-col sm:justify-between pl-22 sm:pl-0">
                            <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                                ...
                                <p className="text-[15px] dark:text-Neutral-0 text-Neutral-500">
                                    {user.joined}
                                </p>
                            </div>
                        </div>
                    ...
                    <section className="mt-6 dark:bg-Neutral-900 bg-Neutral-100 rounded-[10px] p-4 sm:px-8 sm:py-4">
                        <dl className="flex flex-col gap-4 sm:flex-row sm:justify-between">
                            <div className="space-y-2">
                                ...
                                <dd className="stat-number">{user.repos}</dd>
                            </div>
                            <div className="space-y-2">
                                <dt className="stat-label">Followers</dt>
                                <dd className="stat-number">{user.followers}</dd>

                            </div>
                            <div className="space-y-2">
                                <dt className="stat-label">Following</dt>
                                <dd className="stat-number">{user.following}</dd>
                            </div>
                        </dl>
                    </section>
                    {/* Links */}
                    <section className="mt-6 grid gap-4 sm:grid-cols-2 text-[15px]">
                        <ul className="space-y-2">
                            <li className="linkSection">
                                <svg className="w-6 dark:text-Neutral-0 text-Neutral-900" fill="currentColor" viewBox="0 0 24 24">
                                ...
                                <span className="dark:text-Neutral-0 text-Neutral-500">{user.location}</span>
                            </li>
                            <li>
                                <a className="linkSection Focus rounded-xl " target="blank" href={user.website}>
                                    <svg className="w-6  dark:text-Neutral-0 text-Neutral-900" fill="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <span className="dark:text-Neutral-0 text-Neutral-500 break-all">{user.website}</span>
                                </a>
                        ...
                        <ul className="space-y-2 ">
                            <li>
                                <a className="opacity-75 linkSection" href="">
                                    <span className="order-1 dark:text-Neutral-0 text-Neutral-500 opacity-70">{user.twitter}</span>
                                    <svg className="w-6 dark:text-Neutral-0 text-Neutral-900 opacity-70" fill="currentColor" viewBox="0 0 20 18">
                            ...
                            <li className="linkSection  " >
                                ...
                                <span className="dark:text-Neutral-0 text-Neutral-500">{user.company}</span>
                            </li>
                        </ul>
